...
struct DiscoverDetailView: View {
    ...
    private func handleAddToLibrary() {
        ...
        #if DEBUG
        print("[DiscoverDetailView] add -> id=\(detail.id) subtitle=\(detail.subtitle ?? "-") publisher=\(detail.publisher ?? "-") year=\(detail.publishedYear ?? "-") pages=\(detail.pageCount?.description ?? "-") language=\(detail.languageCode ?? "-") cats=\(detail.categories) info=\(detail.infoLink?.absoluteString ?? "-") preview=\(detail.previewLink?.absoluteString ?? "-")")
        #endif
